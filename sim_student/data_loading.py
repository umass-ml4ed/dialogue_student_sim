...
def read_csv(path: str, extra_obj_cols: Optional[List[str]] = None):
    obj_cols = ["turns", "subjects", "gt_turns", "correctness", "ocean_persona", "question_annotation", "eedi_kcs", "acts", "turn_prev"]
    if extra_obj_cols:
...
def process_eedi_dialogues(df: pd.DataFrame):
    ...
    for name, group in df.groupby(["UserId", "InterventionId"]):
        turns: List[Turn] = []
        for _, row in group.iterrows():
            role = "tutor" if row["IsTutor"] else "student"
            turn_text = str(row["anonymized_MessageString"]).replace("\n", " ").strip()
        ...
        tutor_sub = group[group["IsTutor"] == 1]

        data.append({
            # "key": (int(name[0]), int(name[1])),
            # "question": metadata[name]["question"],
            # "subjects": metadata[name]["subjects"],
            "studentID": group.iloc[0]["UserId"],
            "QuestionID": group.iloc[0]["QuestionId"],
            'InterventionId': group.iloc[0]["InterventionId"],
            'tutorID': tutor_sub.iloc[0]["TutorId"],
            "turns": turns
        })
    return data
...
def process_data(data):
    for dialogue in data:
        dia = dialogue["turns"]
        for idx, turn in enumerate(dia):
            if turn["role"] == "student":
                dia[idx]["content"] = turn["content"].split("(Student dialogue act:")[0].strip()
    
    return data


def main():
    df = pd.read_csv('data/annotated/eedi/anonymized-dialogues-with-questions.csv')
    ...
    # cleaned_data = pd.read_csv('data/annotated/eedi/student_dialogue_subset_annotated.csv')
    # cleaned_data.rename(columns={"turns": "turns_before_clean"}, inplace=True)

    cleaned_data = pd.read_csv('data/annotated/eedi/student_dialogue_valid_pair.csv')
    res = pd.merge(cleaned_data, data[["studentID", "InterventionId", 'turns']], on=["studentID", "InterventionId"], how="left")
    print(res.columns)
    res.to_csv('data/annotated/eedi/student_dialogue_clean.csv', index=False)
