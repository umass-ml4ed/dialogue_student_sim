...
from sim_student.data_loading import load_train_val_data, process_data
...
class SFTCombinedDataset(DatasetBase):
    def __init__(self, data: List[Dialogue], role: str, tokenizer: PreTrainedTokenizer, args: dict):
        ...
        for dialogue in data:
            ...
            prompt = get_local_prompt(dialogue, role, tokenizer, persona_type=args["persona"], input_type=args["input_type"])
...
def sft(args):
    # # Exlude the dialogue at t=1 for each student
    # df = pd.read_csv('data/annotated/eedi/student_dialogue_annotated_filt.csv')
    # set_trace()
    # df = df.sort_values(['studentID', 'InterventionId'])
    # df['profile_prev'] = df.groupby('studentID')['gen_profile'].shift(1)
    # df['turn_prev'] = df.groupby('studentID')['turns'].shift(1)
    # df['timestamp'] = df.groupby('studentID').cumcount()
    # df = df[df['profile_prev'].notna()]
    # set_trace()
    ...
    #     wandb.init(project='dialogue_sim')
...
def main():
    ## step 1: match the new problemId in version 2 with the student dialogue result csv
    # data = pd.read_csv('data/annotated/eedi/eedi-interventions-with-questions-v2.csv')
    # data = data.sort_values(by=["UserId", "InterventionId"])
    
    # data = data[data['QuestionId'].notnull()]
    # # data = data[data['MatchConfidence'] == 'high']
    # # data = data[data['NeedsReview'] == False]
    # set_trace()

    # dialogue_df = pd.read_csv('data/annotated/eedi/processed_dialogue.csv')

    # merged = pd.merge(dialogue_df, data[['UserId', 'InterventionId', 'QuestionId']], left_on=['studentID', 'InterventionId'], right_on=['UserId', 'InterventionId'], how="right")
    # merged.to_csv('data/annotated/eedi/student_dialogue_new.csv', index=False)

    # df = pd.read_csv('data/annotated/eedi/student_dialogue_new.csv')
    # set_trace()
    # # step 2: extract the question text for each question and match with the student dialogue csv
    # questions = pd.read_csv('data/annotated/eedi/extracted-question-metadata.csv')
    # questions['Text'] = questions['Text'].fillna("None")
    # questions["line"] = questions["Label"] + ": " + questions['Text']
    # # group into one block per question
    # out = questions.groupby("QuestionId")["line"].apply(lambda s: "\n".join(x for x in s if x.strip())).reset_index(name="question")
    
    # # df = pd.read_csv('data/annotated/eedi/student_dialogue_clean.csv')

    # set_trace()
    # res = pd.merge(df, out, on="QuestionId", how="left")
