...
from sim_student.data_loading import load_train_val_data, load_test_data, process_data
...
def train_acts_classifier(args: dict):
    ...
    # log in to wandb if key provided
    if args["wandb"]:
        wandb.login(key=args["wandb_key"], verify=True)
        wandb.init(project='act_classification')
