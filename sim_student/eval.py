...
def eval_similarity_text(data: List[dict], _args: dict = None, sbert_model: SentenceTransformer = None):
    ...
    _sbert_model = sbert_model or SentenceTransformer("Qwen/Qwen3-Embedding-4B")
...
def apply_defaults(args: dict):
    ...
    if args["level"] == "turn":
        if args["role"] == "student":
            ...
            defaults["evaluations"] = ["sim-text", "sim-acts"]
...
def eval(df: pd.DataFrame, args: dict):
    ...
    if args["level"] == "turn":
        ...
        cols_to_save = ["prompt", "dialogue_idx", "turn_idx", "gt_turn", "pred_turn"]
